...
| 2026-08-19 | 2 (grup A) | musteri-detay · proje-detay · lokasyon-detay · ekipman-detay · _qa/test/detay-render.test.js (yeni) | 0 bulgu / YAPISAL SORUN YOK · 364 test geçti | 43f3dfc | 22 alt liste ListController'a geçti. `lokasyon-detay` ekipman listesindeki "ilk 30 kayıt" kesmesi kaldırıldı — davranış değişti (kesme yerine gerçek sayfalama). Alt liste sayfa boyutu 10'da sabit, boyut seçici açılmadı: kart içi listede seçici kart başlığını ikinci satıra düşürüyor. Fatura/poz/kapsam alt toplamları bilinçli olarak kümenin tamamı üzerinden hesaplanır. |
| 2026-08-19 | 3 (grup B) | is-emri-detay · rapor-detay · uygunsuzluk-detay · teklif-detay · _qa/test/detay-render.test.js | 0 bulgu / YAPISAL SORUN YOK · 372 test geçti | 273cf54 | 15 alt liste daha geçti. **Doküman §6 ile gerçek ekran farkı:** `uygunsuzluk-detay` ve `teklif-detay` ekranlarında tablo biçimli alt koleksiyon YOK — aksiyon planı, kanıt ve doğrulama tek kayıt alanları, tek koleksiyon aşama/onay zinciri. Bu iki ekranda ListController yalnız zincirin yükleniyor/boş/hata sözleşmesini birleştirmek için kullanıldı; sayfa boyutu 50 verilerek zincirin bölünmesi engellendi (pager gizli kalır). `rapor-detay` kapsam tablosu ekipman ve hizmet kalemlerini AYRI kaynaklar olarak tutar, tek listede birleştirmez. |
| 2026-08-19 | 4 (grup C) | sozlesme-detay · fatura-grubu-detay · taseron-detay · personel-detay · cihaz-detay · _qa/test/detay-render.test.js | 0 bulgu / YAPISAL SORUN YOK · 382 test geçti | fe0de57 | **13/13 detay ekranı ListController yolunda.** Toplam 57 alt liste. Tabloya bağlı uyarı notları (mükerrer fatura engeli, belge geçerliliği, fiyat ayrımı, zimmet kalibrasyonu) statik kaba taşındı ve kümenin tamamına bakar — sayfa değiştirince kaybolmazlar. `taseron-detay` fiyat listesi eskiden 460px kapaklı kaydırmadaydı; kapak korundu ama artık 10'arlı sayfalanıyor, yani kapak pratikte devreye girmiyor. Alt toplam satırları her yerde kümenin tamamı üzerinden hesaplanır (sayfa dilimi değil) — bu bilinçli bir sözleşmedir. `personel-detay` ikinci "Belge Yenile" düğmesi artık sonradan çizildiği için dinleyici delegasyonla bağlandı. |
